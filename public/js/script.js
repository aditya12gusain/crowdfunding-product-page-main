...
function bookmarkState(
  ...
  toggle.firstElementChild.firstElementChild.children[0].setAttribute(
    "fill",
    roundButtonColor
  );
  toggle.firstElementChild.firstElementChild.children[1].setAttribute(
    "fill",
    iconColor
  );
...
function updateStatistics(inputPledge) {
  let backed = parseInt(
    document.getElementById("backed").firstElementChild.dataset.target
  );
  ...
  document.getElementById("backed").firstElementChild.textContent =
    total.toLocaleString();
  document.getElementById("backed").firstElementChild.dataset.target = total;
  document.getElementById("backers").textContent = (
    backers + 1
  ).toLocaleString();
...
products.addEventListener("click", (e) => {
  if (e.target.matches("a.button")) {
    ...
    const modalCardSelected = modalDefault.querySelectorAll(
      `section article:nth-of-type(${e.target.dataset.card})`
    )[0];

    modalCardSelected.className =
      "relative mx-auto mb-6 bg-white border border-2 rounded-md border-cyan-100 lg:pt-2 lg:rounded-lg available";
    ...
    modalCardSelected.lastElementChild.classList.replace(
      "border-t-0",
      "border-t-2"
    );
...
function resetDefaultModal() {
  modalDefault.querySelectorAll("article.available").forEach((elem) => {
    elem.className =
      "relative mx-auto mb-6 bg-white border border-gray-300 rounded-md lg:pt-2 lg:rounded-lg available";
